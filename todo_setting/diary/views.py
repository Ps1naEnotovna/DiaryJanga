...
class DiaryPupilGroupView(View):
    def get(self, request):
        group_diary = [
            Diary.objects.filter(pupil=_pupil) for _pupil in Pupil.objects.all()
        ]
        return render(
            request, 'diary/diary_pupil_group.html', {'group_diary': group_diary}
        )
...
class DiarySubjectGroupView(View):
    def get(self, request):
        group_subject_diary = [
            Diary.objects.filter(subject=_subject)
            for _subject in Subject.objects.filter(diary__isnull=False).distinct()
        ]
        return render(
            request,
            'diary/diary_subject_group.html',
            {'group_subject_diary': group_subject_diary},
        )
...
class DiaryDateGroupView(View):
    def get(self, request: WSGIRequest):
        query = Diary.objects
        if (order_by := request.GET.get('order_by')) and order_by in Diary.__doc__[
            len(Diary.__name__) + 1 : -1
        ].split(', '):
            query = query.order_by(order_by)
        group_date_diary = [
            query.filter(date=_date)
